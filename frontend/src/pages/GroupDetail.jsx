...
export default function GroupDetail({ groupId, group, onBack, initialTab = 'expenses' }) {
  const { auth } = useAuth();
...
function MembersTab({ groupId, members, balances = [], onAdded }) {
  ...
  return (
    <div>
      ...
      <div className="card divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
        {members.map((m) => {
          const bal = balances.find((b) => Number(b.userId) === Number(m.userId));
  ...
  if (settlements.length === 0) {
    return (
      <div className="card border-brand-200 dark:border-brand-800 bg-gradient-to-b from-brand-50 to-white dark:from-brand-900/20 dark:to-slate-900 rounded-2xl py-14 px-6 text-center animate-fade-in">
        ...
        <h3 className="text-lg font-extrabold text-brand-700 dark:text-brand-300">You&apos;re all settled.</h3>
        <p className="mt-0.5 font-mono text-3xl font-extrabold text-slate-900 dark:text-white">₹0</p>
